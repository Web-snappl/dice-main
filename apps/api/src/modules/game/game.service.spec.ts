...
describe('GameService', () => {
  const makeService = (overrides?: {
    ...
    const gameHistoryModel = function () {
      return { save: jest.fn() };
    } as any;
    const gameConfigModel = overrides?.gameConfigModel ?? {
      findOneAndUpdate: jest.fn(() => ({
        exec: jest.fn().mockResolvedValue({ gameId: 'dice_duel' }),
      })),
      findOne: jest.fn(() => ({
        exec: jest.fn().mockResolvedValue({ gameId: 'dice_duel' }),
      })),
  ...
  it('uses configured commission rate in payout calculation (not hardcoded value)', async () => {
    ...
    const users = [
      {
        uid: '64b7f4be5f99f4f1b4f12345',
        displayName: 'Winner',
        betAmount: 100,
        winner: true,
      },
      {
        uid: '64b7f4be5f99f4f1b4f67890',
        displayName: 'Loser',
        betAmount: 100,
        winner: false,
      },
  ...
  it('does not throw when winner is last player in multi-player mapping', async () => {
    ...
    expect(winner?.winsAgainst).toEqual(
      expect.arrayContaining(['user_1', 'opponent_1']),
    );
  });
});
